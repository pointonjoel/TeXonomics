...
\begin{tikzpicture}[thick,font=\sffamily,scale=1.5]
	%axis
	\draw (0,4) node[left]{Inflation} -- (0,0) node[below left] {$0$} -- (4,0) node[below right]{Real GDP};
	...
	%dots and labels
	\node[style={fill=black,circle,inner sep=0pt,minimum size=4pt}] at (2,1.5) { }; %A - equib
	\node[style={fill=black,circle,inner sep=0pt,minimum size=4pt}] at (2.25,1.75) { }; %B- SR equib
	\node[style={fill=black,circle,inner sep=0pt,minimum size=4pt}] at (2,2) { }; %C- new LR equib
	
	\node[below left]at (1.9,1.55) {\tiny{A}}; %A label
	\node[right]at (2.25,1.75) {\tiny{B}}; %B label
	\node[above right]at (2.1,1.95) {\tiny{C}}; %C label
\end{tikzpicture}
\end{document}